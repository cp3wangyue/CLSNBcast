...
| 音频 | 1 | **0.57** | 0.57 | ✅ 一致 |
| HD 高清（含 SD） | 4 | **2** | 2 | ✅ 一致 |
| Full HD 全高清 | 9 | **4.5** | 4.57 | ❌ **上游有误，应为 4.5** |
| 2K | 16 | 7.8 或 8（3:2 分歧） | 8 | ⚠️ 未定 |
| 2K+ 超高清 | 36 | 18 或 8（3:2 分歧） | 18 | ⚠️ 未定，18 更可信 |
...
1. **Phase 2 交付时**，`quality_config` 的系数默认值取官方确认值：音频 `0.57` / HD `2` / Full HD `4.5` / 2K `8` / 2K+ `18`。
...
- [ ] 后付费单价是否仍为 **7 元 / 1000 标准分钟**（0.007 元/分钟）？我无法从公开文档确认，且各账户可能有差异。
- [ ] 是否存在我们未建模的计费项（录制、转码、CDN 推流等）？
